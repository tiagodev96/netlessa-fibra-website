...
const Contact: FC<{ phoneNumber: string; email: string }> = ({ phoneNumber, email }) => {
    ...
    {socials.map((social) => (
      <div key={social.id}>
        <Link href={social.link} target="_blank">
          <Image src={social.icon.url ?? ''} alt={social.social_name} width={24} height={24} />
        </Link>
      </div>
    ))}
  </div>
...
const CompanyInfos: FC<CompanyInfosProps> = ({ logo, address, phoneNumber, email, socials }) => {
  return (
